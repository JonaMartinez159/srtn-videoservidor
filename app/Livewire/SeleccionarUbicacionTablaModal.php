...
class SeleccionarUbicacionTablaModal extends Component
{
    ...
    public function enviarOtraUbicacion(){
        ...
        $nuevo_sitio->calle = $this->calle;
        $nuevo_sitio->colonia = $this->colonia;
        $nuevo_sitio->numero = $this->numero;

        $nuevo_sitio->etiqueta = 'otro';
        $nuevo_sitio->save();
        
        $this->dispatch('UbicacionSeleccionada', ubicacion_seleccionada: $nuevo_sitio->id);
        $this->showModalubicacion=false;

        //indicarle al mapa que no pudo obtener la ubicacion
        $this->otro_seleccionado = true;
    }
